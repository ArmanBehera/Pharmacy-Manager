...
class MedicineStockSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        medicine_data = validated_data['medicine']
        medicine = Medicines.objects.create(**medicine_data)

        if not medicine:
            raise serializers.ValidationError('Failed to create medicine object.')

        validated_data['medicine'] = medicine

        try:
            medicineObject = MedicineStock.objects.create(**validated_data)
            return medicineObject 
        except Exception as e:
            medicine.delete()
            return serializers.ValidationError('Failed to create medicine object.')
